...
test('7 manager may write orders but not expenses', async () => { await assertSucceeds(setDoc(doc(db('companyAManager'), `${company('companyA')}/orders/manager`), { companyId: 'companyA' })); await assertFails(getDoc(doc(db('companyAManager'), `${company('companyA')}/expenses/expenseA`))); });
