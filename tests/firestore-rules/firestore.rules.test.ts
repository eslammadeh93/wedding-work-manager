...
test('7 manager has the same operational access as the company owner', async () => { await assertSucceeds(setDoc(doc(db('companyAManager'), `${company('companyA')}/orders/manager`), { companyId: 'companyA' })); await assertSucceeds(getDoc(doc(db('companyAManager'), `${company('companyA')}/expenses/expenseA`))); });
