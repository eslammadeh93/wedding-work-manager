...
const legacyPermissions = (role: CompanyRole): readonly CompanyPermission[] => role === 'company_super_admin' ? COMPANY_PERMISSIONS : role === 'manager' ? COMPANY_PERMISSIONS.filter(permission => !permission.startsWith('company:expenses:')) : role === 'employee' ? ['company:dashboard:read', 'company:calculator:use', 'company:calendar:read', 'company:orders:read', 'company:orders:write', 'company:customers:read', 'company:customers:write', 'company:inventory:read', 'company:notifications:read'] : ['company:orders:read', 'company:worker_performance:read', 'company:notifications:read'];
