...
const COMPANY_PERMISSIONS: readonly CompanyPermission[] = ['company:dashboard:read', 'company:calculator:use', 'company:calculator:manage', 'company:order_responsibles:manage', 'company:calendar:read', 'company:orders:read', 'company:orders:write', 'company:customers:read', 'company:customers:write', 'company:suppliers:read', 'company:suppliers:write', 'company:workers:read', 'company:workers:write', 'company:worker_performance:read', 'company:inventory:read', 'company:inventory:write', 'company:expenses:read', 'company:expenses:write', 'company:categories:read', 'company:categories:write', 'company:activity_logs:read', 'company:reports:read', 'company:settings:read', 'company:settings:write', 'company:members:read', 'company:members:write', 'company:notifications:read'];
const legacyPermissions = (role: CompanyRole): readonly CompanyPermission[] => role === 'company_super_admin' ? COMPANY_PERMISSIONS : role === 'manager' ? COMPANY_PERMISSIONS.filter(permission => !permission.startsWith('company:expenses:')) : role === 'employee' ? ['company:dashboard:read', 'company:calculator:use', 'company:calendar:read', 'company:orders:read', 'company:orders:write', 'company:customers:read', 'company:customers:write', 'company:inventory:read', 'company:notifications:read'] : ['company:calendar:read', 'company:orders:read', 'company:worker_performance:read', 'company:notifications:read'];
const memberPermissions = (member: MemberDoc): readonly CompanyPermission[] => Array.from(new Set([
  ...(Array.isArray(member.permissions) ? member.permissions : legacyPermissions(member.role || 'worker')),
  ...(['company_super_admin', 'manager', 'worker'].includes(member.role || 'worker') ? ['company:worker_performance:read' as CompanyPermission] : []),
  ...(member.role === 'worker' ? ['company:calendar:read' as CompanyPermission] : []),
]));
...
export class CompanyMemberService {
  ...
  async create(input: unknown, auth: AuthContext): Promise<CreateCompanyMemberResponse> {
    ...
    const storedMaxUsers = context.company.data()?.maxUsers;
    const maxUsers = storedMaxUsers === null ? null : Number(storedMaxUsers);
    if (maxUsers !== null && (!Number.isInteger(maxUsers) || maxUsers < 1)) return fail('INVALID_INPUT', 'إعداد maxUsers للشركة غير صالح.');
  ...
  async setWorkerStatus(input: unknown, auth: AuthContext): Promise<SetWorkerStatusResponse> {
    ...
    try { await this.deps.auth.updateUser(authUid, { disabled: !active }); await this.deps.db.runTransaction(async tx => { tx.update(workerRef, { status: data.status, updatedAt: admin.firestore.FieldValue.serverTimestamp() }); tx.update(memberRef, { status: active ? 'active' : 'disabled', updatedAt: admin.firestore.FieldValue.serverTimestamp() }); }); if (!active) await this.deps.auth.revokeRefreshTokens(authUid); }
