...
import { ArrowRight, Calculator, ExternalLink, LoaderCircle, MapPin, Pencil, Plus, Route, Settings2, Trash2, X } from 'lucide-react';
...
type CalculatorView = 'chooser' | 'order-settings' | 'transportation';

export const OrderCalculatorModule: React.FC<{ onOpenCalculator: () => void }> = ({ onOpenCalculator }) => {
  const { authSession } = useAuth();
  const { settings, updateSettings } = useData();
  const [view, setView] = useState<CalculatorView>('chooser');
  ...
  const saveCatalogItem = async () => {
    ...
    if (!canManage || !cleanName || !Number.isFinite(unitPrice) || unitPrice < 0) return;
  ...
  const saveLocation = async () => {
    ...
    if (!canManage || !cleanName || !cleanUrl) return;
    try {
  ...
  return <div dir="rtl" className="mx-auto max-w-5xl space-y-6 animate-in fade-in duration-300">
    <header className="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
      <div className="flex flex-wrap items-center gap-3">
        {view !== 'chooser' && <button type="button" onClick={() => setView('chooser')} className="flex items-center gap-1 rounded-xl border border-slate-200 px-3 py-2 text-xs font-black text-slate-600 hover:bg-slate-50 dark:border-slate-700 dark:text-slate-200"><ArrowRight className="h-4 w-4" />كل الحاسبات</button>}
        <div className="flex items-center gap-3"><div className="flex h-12 w-12 items-center justify-center rounded-2xl bg-emerald-100 text-emerald-700 dark:bg-emerald-950/50 dark:text-emerald-300"><Calculator className="h-6 w-6" /></div><div><h1 className="text-xl font-black text-slate-900 dark:text-white">{view === 'transportation' ? 'حاسبة الانتقالات' : view === 'order-settings' ? 'إعدادات حاسبة الأوردرات' : 'الحاسبات'}</h1><p className="mt-0.5 text-xs text-slate-500">{view === 'chooser' ? 'اختر الحاسبة التي تريد استخدامها.' : view === 'transportation' ? 'احسب الذهاب والعودة من روابط Google Maps.' : 'أضف الخدمات أو المكوّنات وأسعارها.'}</p></div></div>
      </div>
    </header>
