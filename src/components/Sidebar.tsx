...
export const Sidebar: React.FC<SidebarProps> = ({
  ...
  const allNavItems: { id: ActiveTab; group: NavigationGroup; labelKey?: keyof typeof import('../i18n/translations').translations['en']; label?: string; icon: React.FC<{ className?: string }>; badge?: number; roles: ('super_admin' | 'admin' | 'manager' | 'employee' | 'worker')[]; permission?: Permission }[] = [
    ...
    { id: 'calendar', group: 'sales', label: language === 'ar' ? (userRole === 'worker' ? 'تقويم تركيباتي' : 'تقويم التركيبات') : (userRole === 'worker' ? 'My Installation Calendar' : 'Installation Calendar'), icon: CalendarDays, roles: ['super_admin', 'admin', 'manager', 'employee', 'worker'], permission: 'company:calendar:read' },
