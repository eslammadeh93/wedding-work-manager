      ...
      <div className="max-h-[92vh] w-full max-w-3xl overflow-hidden rounded-3xl border border-slate-200 bg-white shadow-2xl dark:border-slate-800 dark:bg-slate-900" onClick={event => event.stopPropagation()}>
        ...
        <form onSubmit={submit} className="max-h-[calc(92vh-89px)] overflow-y-auto p-5 sm:p-6">
          <div className="grid grid-cols-1 gap-4 sm:grid-cols-2">
            ...
            <Field label="رقم الهاتف *"><input required value={form.phone} onChange={e => set('phone', sanitizePhoneInput(e.target.value))} className={inputClass} inputMode="tel" placeholder="01xxxxxxxxx" /></Field>
            <Field label="رقم واتساب (إن كان مختلفًا)"><input value={form.whatsapp} onChange={e => set('whatsapp', sanitizePhoneInput(e.target.value))} className={inputClass} inputMode="tel" placeholder="اتركه فارغًا لاستخدام رقم الهاتف" /></Field>
            <Field label="رقم بديل"><input value={form.secondaryPhone} onChange={e => set('secondaryPhone', sanitizePhoneInput(e.target.value))} className={inputClass} inputMode="tel" placeholder="رقم إضافي" /></Field>
