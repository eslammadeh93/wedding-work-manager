    ...
    <div className="p-4 bg-slate-50 dark:bg-slate-800/80 rounded-2xl border border-slate-200 dark:border-slate-700 space-y-2">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          {(order.responsibleName || order.salesEmployee) && !isWorker && <p className="text-xs text-indigo-600 dark:text-indigo-400 flex items-center gap-1 mt-1 font-semibold"><UserCheck className="w-3.5 h-3.5" /><span>{language === 'ar' ? 'المسؤول عن الأوردر' : 'Order responsible'}: {order.responsibleName || order.salesEmployee}{order.responsiblePhone ? ` · ${order.responsiblePhone}` : ''}</span></p>}
