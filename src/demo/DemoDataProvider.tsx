...
export function DemoDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const updateOrderPayments = useCallback(async (id: string, apply: (history: PaymentEntry[]) => PaymentEntry[] | null) => { const order = store.orders.find(item => item.id === id); if (!order) throw new Error('لم يتم العثور على الطلب.'); const nextHistory = apply(order.paymentHistory || []); if (!nextHistory) return; await updateOrder(id, { paymentHistory: nextHistory }); }, [store.orders, updateOrder]);
  const addPaymentToOrder = useCallback(async (id: string, payment: Omit<PaymentEntry, 'id'> & { id?: string }) => { const entry: PaymentEntry = { ...payment, id: payment.id || newId('demo_payment') }; await updateOrderPayments(id, history => appendPaymentEntry(history, entry)); }, [updateOrderPayments]);
  ...
  const value = useMemo<DataContextType>(() => ({
    orders: activeOrders,
    // The demo store is held entirely in memory, so its accounting dataset is
    // the same complete set and is always available.
    ...
    // The demo store is in memory and carries no legacy records, so there is
    // nothing to survey and nothing to repair.
    ...
    addOrder, updateOrder, updateOrderPayments, deleteOrder, addPaymentToOrder, addWorkTask, updateWorkTask, deleteWorkTask, addWorker, updateWorker, deleteWorker: async (id) => update(current => ({ ...current, workers: current.workers.filter(item => item.id !== id) })), toggleWorkerStatus: async (id, status) => updateWorker(id, { status }),
  ...
  }), [activeCustomers, activeInventory, activeOrders, addActivityLog, addCategory, addOrder, addPaymentToOrder, addRecord, addWorkTask, exportBackupJson, recycleBinItems, restoreBackupJson, restoreDeletedItem, store, totals, update, updateOrder, updateOrderPayments, updateRecord, updateWorkTask, updateWorker]);
