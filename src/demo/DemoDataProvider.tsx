...
export function DemoDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const addPaymentToOrder = useCallback(async (id: string, payment: Omit<PaymentEntry, 'id'> & { id?: string }) => { const order = store.orders.find(item => item.id === id); if (!order) throw new Error('لم يتم العثور على الطلب.'); const paymentId = payment.id || newId('demo_payment'); if ((order.paymentHistory || []).some(entry => entry.id === paymentId)) return; await updateOrder(id, { paymentHistory: [...(order.paymentHistory || []), { ...payment, id: paymentId }] }); }, [store.orders, updateOrder]);
