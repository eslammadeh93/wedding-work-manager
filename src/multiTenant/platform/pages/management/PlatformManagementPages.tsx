    ...
    try {
      ...
      if (page === "admins") {
        ...
        setAdmins(adminSnapshot.docs.map(doc => {
          const data = doc.data(); return { uid: doc.id, name: textValue(data.name) || "بدون اسم", email: textValue(data.email) || "—", role: textValue(data.role) || "platform_read_only", status: textValue(data.status) || "active", permissions: Array.isArray(data.permissions) ? data.permissions.filter((permission): permission is string => typeof permission === "string") : undefined, permissionsCustomized: data.permissionsCustomized === true, createdAt: data.createdAt };
        }));
      }
...
function PlansPage() {
  const { authSession } = useAuth();
  const canManagePlans = authSession?.userType === "platform" && authSession.permissions.includes("platform:plans:manage");
  const [plans, setPlans] = useState<PlatformPlan[]>([]); const [loading, setLoading] = useState(true); const [error, setError] = useState(""); const [formOpen, setFormOpen] = useState(false); const [editing, setEditing] = useState<PlatformPlan | null>(null); const [name, setName] = useState(""); const [unlimited, setUnlimited] = useState(false); const [maxUsers, setMaxUsers] = useState(1); const [saving, setSaving] = useState(false);
  const loadPlans = useCallback(async () => { setLoading(true); setError(""); try { setPlans(await listPlatformPlans()); } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر تحميل الباقات."); } finally { setLoading(false); } }, []);
  useEffect(() => { void loadPlans(); }, [loadPlans]);
  const openCreate = () => { setEditing(null); setName(""); setUnlimited(false); setMaxUsers(1); setFormOpen(true); };
  const openEdit = (plan: PlatformPlan) => { setEditing(plan); setName(plan.name); setUnlimited(plan.maxUsers === null); setMaxUsers(plan.maxUsers || 1); setFormOpen(true); };
  const save = async () => { const limit = unlimited ? null : maxUsers; if (name.trim().length < 2 || (!unlimited && (!Number.isInteger(limit) || limit < 1))) return; setSaving(true); setError(""); try { if (editing) await updatePlatformPlan({ planId: editing.id, name: name.trim(), maxUsers: limit }); else await createPlatformPlan({ name: name.trim(), maxUsers: limit }); setFormOpen(false); await loadPlans(); } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر حفظ الباقة."); } finally { setSaving(false); } };
  return <section><PlatformPageHeader title="الباقات" description="أنشئ باقات الاشتراك وحدد عدد الموظفين المسموح به لكل شركة." actions={canManagePlans ? <PlatformButton onClick={openCreate}>إضافة باقة</PlatformButton> : undefined} />{error && <p className="mb-4 rounded-xl bg-rose-100 p-3 text-sm font-bold text-rose-800 dark:bg-rose-950 dark:text-rose-100">{error}</p>}{loading ? <LoadingState text="جارٍ تحميل الباقات…" /> : plans.length ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{plans.map(plan => <PlatformCard key={plan.id} className="p-5"><div className="flex items-start justify-between gap-3"><div><h2 className="font-black">{plan.name}</h2><p className="mt-2 text-sm text-slate-500">{plan.maxUsers === null ? "عدد موظفين غير محدود" : `حتى ${plan.maxUsers} موظف`}</p></div>{canManagePlans && <PlatformButton variant="secondary" onClick={() => openEdit(plan)}>تعديل</PlatformButton>}</div></PlatformCard>)}</div> : <EmptyState text="لا توجد باقات بعد. أضف أول باقة لتظهر في الاشتراكات." />}{formOpen && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto mt-20 max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900"><h2 className="font-black">{editing ? "تعديل الباقة" : "إضافة باقة"}</h2><div className="mt-4 space-y-4"><label className="block text-sm font-bold">اسم الباقة<input className={`${fieldClass} mt-1`} value={name} onChange={event => setName(event.target.value)} placeholder="مثال: باقة الأعمال" /></label><label className="flex items-center gap-2 rounded-xl bg-slate-50 p-3 text-sm font-bold dark:bg-slate-800"><input type="checkbox" checked={unlimited} onChange={event => setUnlimited(event.target.checked)} className="h-4 w-4 accent-amber-600" />عدد الموظفين غير محدود</label>{!unlimited && <label className="block text-sm font-bold">عدد الموظفين المسموح<input type="number" min="1" step="1" className={`${fieldClass} mt-1`} value={maxUsers} onChange={event => setMaxUsers(Number(event.target.value))} /></label>}</div><div className="mt-5 flex gap-2"><PlatformButton disabled={saving || name.trim().length < 2 || (!unlimited && (!Number.isInteger(maxUsers) || maxUsers < 1))} onClick={() => void save()}>{saving ? "جارٍ الحفظ…" : "حفظ الباقة"}</PlatformButton><PlatformButton variant="secondary" disabled={saving} onClick={() => setFormOpen(false)}>إلغاء</PlatformButton></div></div></div>}</section>;
}
...
function SubscriptionsPage({ companies, load, navigate }: DataProps) {
  ...
  const [plans, setPlans] = useState<PlatformPlan[]>([]); const [plansLoading, setPlansLoading] = useState(false); const [planId, setPlanId] = useState(""); const [subscriptionEnd, setSubscriptionEnd] = useState(""); const [nextStatus, setNextStatus] = useState<"trial" | "active" | "past_due" | "expired" | "suspended">("active");
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const open = async (company: PlatformCompany) => { setEditing(company); setPlanId(company.planId || ""); setSubscriptionEnd(typeof company.subscriptionEnd === "string" ? company.subscriptionEnd.slice(0, 10) : ""); setNextStatus(company.status); setError(""); setPlansLoading(true); try { const availablePlans = await listPlatformPlans(); setPlans(availablePlans); setPlanId(company.planId || availablePlans.find(plan => plan.name === company.plan)?.id || ""); } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر تحميل الباقات."); } finally { setPlansLoading(false); } };
  const save = async () => {
    if (!editing || !planId || !subscriptionEnd) return;
    setSaving(true); setError("");
    try { await managePlatformSubscription({ companyId: editing.id, planId, status: nextStatus, subscriptionEnd }); setEditing(null); await load(); }
  ...
  return <section><PlatformPageHeader title="الاشتراكات" description="متابعة الباقات وتواريخ الانتهاء وحالة كل اشتراك." />
    ...
    {editing && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto mt-16 max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900"><h2 className="font-black">إدارة اشتراك {editing.name}</h2><div className="mt-4 space-y-3"><label className="block text-sm font-bold">الباقة<select className={`${fieldClass} mt-1`} value={planId} disabled={plansLoading} onChange={e => setPlanId(e.target.value)}><option value="">{plansLoading ? "جارٍ تحميل الباقات…" : "اختر الباقة"}</option>{plans.map(plan => <option key={plan.id} value={plan.id}>{plan.name} — {plan.maxUsers === null ? "موظفون غير محدودين" : `حتى ${plan.maxUsers} موظف`}</option>)}</select></label>{!plansLoading && !plans.length && <p className="rounded-xl bg-amber-100 p-3 text-sm font-bold text-amber-900 dark:bg-amber-950 dark:text-amber-100">أضف باقة أولًا من صفحة الباقات.</p>}<label className="block text-sm font-bold">الحالة<select className={`${fieldClass} mt-1`} value={nextStatus} onChange={e => setNextStatus(e.target.value as typeof nextStatus)}><option value="trial">تجريبية</option><option value="active">نشطة</option><option value="past_due">متأخرة</option><option value="expired">منتهية</option><option value="suspended">موقوفة</option></select></label><label className="block text-sm font-bold">نهاية الاشتراك<input type="date" className={`${fieldClass} mt-1`} value={subscriptionEnd} onChange={e => setSubscriptionEnd(e.target.value)} /></label></div><div className="mt-5 flex gap-2"><PlatformButton disabled={saving || plansLoading || !planId || !subscriptionEnd} onClick={() => void save()}>{saving ? "جارٍ الحفظ…" : "حفظ التعديل"}</PlatformButton><PlatformButton variant="secondary" onClick={() => setEditing(null)}>إلغاء</PlatformButton></div></div></div>}</section>;
}
...
function AdminsPage({ admins, load }: DataProps) {
  ...
  const [formOpen, setFormOpen] = useState(false); const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [password, setPassword] = useState(""); const [role, setRole] = useState<PlatformRole>("platform_admin"); const [createPermissions, setCreatePermissions] = useState<string[]>(() => [...defaultPlatformRolePermissions.platform_admin]); const [createPermissionsCustomized, setCreatePermissionsCustomized] = useState(false);
  const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  ...
  const rolePermissionsRequest = useRef(0);
  const loadRolePermissions = useCallback(async () => {
    const requestId = ++rolePermissionsRequest.current;
    try {
      const saved = await getPlatformPermissionConfiguration();
      if (requestId === rolePermissionsRequest.current) setRolePermissions(saved);
      return saved;
    } catch {
      return null;
    }
  }, []);
  useEffect(() => { if (isOwner) void loadRolePermissions(); }, [isOwner, loadRolePermissions]);
  const togglePermission = (permission: string, set: Dispatch<SetStateAction<string[]>>) => set(current => {
    if (permission === "platform:plans:manage") return current.includes(permission) ? current.filter(item => item !== permission) : [...new Set([...current, "platform:plans:read", permission])];
    if (permission === "platform:plans:read" && current.includes(permission)) return current.filter(item => item !== "platform:plans:read" && item !== "platform:plans:manage");
    return current.includes(permission) ? current.filter(item => item !== permission) : [...current, permission];
  });
  const openCreateForm = () => { const initialRole: PlatformRole = "platform_admin"; setName(""); setEmail(""); setPassword(""); setRole(initialRole); setCreatePermissions([...(rolePermissions[initialRole] || [])]); setCreatePermissionsCustomized(false); setFormOpen(true); };
  const create = async () => { if (name.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 12) return; setSaving(true); setError(""); try { await createPlatformAdmin({ name: name.trim(), email: email.trim(), password, role, useRolePermissions: !createPermissionsCustomized, ...(createPermissionsCustomized ? { permissions: createPermissions } : {}) }); setFormOpen(false); setName(""); setEmail(""); setPassword(""); setCreatePermissionsCustomized(false); await load(); } catch { setError("تعذر إنشاء حساب المشرف. تحقّق من نشر Cloud Functions وصلاحيات حساب صاحب المنصة ثم أعد المحاولة."); } finally { setSaving(false); } };
  ...
  const saveRolePermissions = async () => { if (!editingRole) return; setSaving(true); setError(""); try { await updatePlatformRolePermissions({ role: editingRole, permissions: roleDraft }); const saved = await loadRolePermissions(); setRolePermissions(current => ({ ...current, [editingRole]: saved?.[editingRole] || roleDraft })); setEditingRole(null); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر حفظ صلاحيات المنصب."); } finally { setSaving(false); } };
  ...
  const permissionEditor = (selected: string[], set: Dispatch<SetStateAction<string[]>>, onChange?: () => void) => <div className="grid gap-2 sm:grid-cols-2">{PLATFORM_PERMISSIONS.map(permission => <label key={permission} className="flex items-center gap-2 rounded-lg border border-slate-200 p-2 text-xs font-bold dark:border-slate-700"><input type="checkbox" checked={selected.includes(permission)} onChange={() => { onChange?.(); togglePermission(permission, set); }} className="h-4 w-4 accent-amber-600" />{PLATFORM_PERMISSION_LABELS[permission]}</label>)}</div>;
  ...
  return <section><PlatformPageHeader title="المشرفون والصلاحيات" description="الحسابات الإدارية المعتمدة ونطاق وصول كل دور في المنصة." actions={isOwner ? <PlatformButton onClick={openCreateForm}>إضافة مشرف</PlatformButton> : undefined} />
    ...
    {formOpen && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto mt-10 max-w-3xl rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900"><h2 className="font-black">إضافة مشرف للمنصة</h2><div className="mt-4 space-y-3"><input className={fieldClass} value={name} onChange={e => setName(e.target.value)} placeholder="الاسم" /><input type="email" className={fieldClass} value={email} onChange={e => setEmail(e.target.value)} placeholder="البريد الإلكتروني" /><input type="password" className={fieldClass} value={password} onChange={e => setPassword(e.target.value)} placeholder="كلمة مرور مؤقتة — 12 حرفًا على الأقل" /><label className="block text-sm font-bold">المنصب<select className={`${fieldClass} mt-1`} value={role} onChange={e => { const next = e.target.value as PlatformRole; setRole(next); setCreatePermissions([...(rolePermissions[next] || [])]); setCreatePermissionsCustomized(false); }}>{roles.map(item => <option key={item} value={item}>{roleLabel[item]}</option>)}</select></label></div><div className="mt-5 rounded-xl border border-slate-200 p-4 dark:border-slate-700"><div className="flex flex-wrap items-start justify-between gap-2"><div><h3 className="font-black">صلاحيات {roleLabel[role]}</h3><p className="mt-1 text-sm text-slate-500">كل صلاحيات المنصب مفعّلة تلقائيًا. أزل علامة أي صلاحية لا تريد منحها لهذا المشرف.</p></div><PlatformButton type="button" variant="secondary" disabled={saving} onClick={() => { setCreatePermissions([...(rolePermissions[role] || [])]); setCreatePermissionsCustomized(false); }}>إعادة تفعيل الكل</PlatformButton></div><div className="mt-4">{permissionEditor(createPermissions, setCreatePermissions, () => setCreatePermissionsCustomized(true))}</div></div><div className="mt-5 flex gap-2"><PlatformButton disabled={saving || name.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 12} onClick={() => void create()}>{saving ? "جارٍ الإنشاء…" : "إنشاء الحساب"}</PlatformButton><PlatformButton variant="secondary" disabled={saving} onClick={() => setFormOpen(false)}>إلغاء</PlatformButton></div></div></div>}</section>;
}
