...
function AdminsPage({ admins, load }: DataProps) {
  ...
  const columns: DataTableColumn<PlatformUser>[] = [
    ...
    { key: "role", header: "الدور", render: row => <select aria-label="دور المشرف" disabled={saving} className={`${fieldClass} w-auto`} value={row.role} onChange={e => void update(row, row.status as "active" | "disabled", e.target.value as typeof role)}>{roles.map(item => <option key={item} value={item}>{roleLabel[item]}</option>)}</select> },
    ...
    { key: "status", header: "الحالة", render: row => status(row.status) },
    { key: "date", header: "تاريخ الإنشاء", render: row => formatPlatformDate(row.createdAt) },
    { key: "action", header: "إجراء", render: row => <PlatformButton variant={row.status === "active" ? "danger" : "secondary"} disabled={saving} onClick={() => void update(row, row.status === "active" ? "disabled" : "active")}>{row.status === "active" ? "تعطيل" : "تفعيل"}</PlatformButton> },
  ];
