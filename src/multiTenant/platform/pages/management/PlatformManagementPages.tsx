...
import { PLATFORM_PERMISSION_MATRIX, PLATFORM_ROLES, isPlatformRole, platformRoleHasPermission, type PlatformPermission } from "../../permissions/platformPermissions";
...
type PlatformUser = { uid: string; name: string; email: string; role: string; status: string; createdAt?: unknown };
...
function usePlatformCapability(permission: PlatformPermission) {
  ...
  return authSession?.userType === "platform" && isPlatformRole(role) && platformRoleHasPermission(role, permission);
}
...
function AdminsPage({ admins, load }: DataProps) {
  const { authSession } = useAuth();
  const [formOpen, setFormOpen] = useState(false); const [name, setName] = useState(""); const [email, setEmail] = useState(""); const [password, setPassword] = useState("");
  const roles = ["platform_owner", "platform_admin", "platform_support", "platform_billing", "platform_read_only"] as const;
  const [role, setRole] = useState<(typeof roles)[number]>("platform_admin"); const [saving, setSaving] = useState(false); const [error, setError] = useState("");
  const [editing, setEditing] = useState<PlatformUser | null>(null); const [editName, setEditName] = useState(""); const [editEmail, setEditEmail] = useState(""); const [editRole, setEditRole] = useState<(typeof roles)[number]>("platform_admin"); const [editStatus, setEditStatus] = useState<"active" | "disabled">("active");
  ...
  const update = async (admin: PlatformUser, status: "active" | "disabled", nextRole = admin.role as typeof role) => { setSaving(true); setError(""); try { await updatePlatformAdmin({ uid: admin.uid, role: nextRole, status }); await load(); } catch { setError("تعذر تحديث حساب المشرف."); } finally { setSaving(false); } };
  const openEdit = (admin: PlatformUser) => { setEditing(admin); setEditName(admin.name); setEditEmail(admin.email); setEditRole(admin.role as typeof editRole); setEditStatus(admin.status as "active" | "disabled"); };
  const saveEdit = async () => { if (!editing || editName.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(editEmail)) return; setSaving(true); setError(""); try { await updatePlatformAdmin({ uid: editing.uid, name: editName.trim(), email: editEmail.trim(), role: editRole, status: editStatus }); setEditing(null); await load(); } catch (cause) { setError(cause instanceof Error ? cause.message : "تعذر تعديل حساب المشرف."); } finally { setSaving(false); } };
  ...
  const columns: DataTableColumn<PlatformUser>[] = [
    ...
    { key: "role", header: "الدور", render: row => roleLabel[row.role] || row.role },
    { key: "permissions", header: "نطاق الصلاحيات", render: row => <PlatformBadge className="platform-badge-neutral">{(PLATFORM_PERMISSION_MATRIX[row.role as keyof typeof PLATFORM_PERMISSION_MATRIX] || []).length} صلاحية</PlatformBadge> },
    { key: "status", header: "الحالة", render: row => status(row.status) },
    { key: "date", header: "تاريخ الإنشاء", render: row => formatPlatformDate(row.createdAt) },
  ...
  return <section><PlatformPageHeader title="المشرفون والصلاحيات" description="الحسابات الإدارية المعتمدة ونطاق وصول كل دور في المنصة." actions={<PlatformButton onClick={() => setFormOpen(true)}>إضافة مشرف</PlatformButton>} />
    <div className="mb-5 grid gap-3 sm:grid-cols-2"><StatCard label="حسابات إدارة المنصة" value={admins.length} icon={<ShieldCheck size={18} />} /><StatCard label="الحسابات النشطة" value={admins.filter(a => a.status === "active").length} icon={<CheckCircle2 size={18} />} /></div>
    {error && <p className="mb-4 rounded-xl bg-rose-100 p-3 text-sm font-bold text-rose-800 dark:bg-rose-950 dark:text-rose-100">{error}</p>}
    {admins.length ? <DataTable rows={admins} columns={columns} rowKey={row => row.uid} minWidthClass="min-w-[820px]" /> : <EmptyState text="لا توجد حسابات إدارة مسجلة." />}
    <PlatformCard className="mt-5 p-5"><h2 className="font-black">مصفوفة الصلاحيات</h2><div className="mt-4 grid gap-3 md:grid-cols-2">{PLATFORM_ROLES.map(role => <div key={role} className="rounded-xl border border-slate-200 p-3 dark:border-slate-700"><p className="font-bold">{roleLabel[role]}</p><p className="mt-1 text-sm text-slate-500">{PLATFORM_PERMISSION_MATRIX[role].length} صلاحية مفعّلة</p></div>)}</div></PlatformCard>
    {editing && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto mt-16 max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900"><h2 className="font-black">تعديل حساب المشرف</h2><p className="mt-1 text-sm text-slate-500">{editing.email.trim().toLowerCase() === protectedPlatformAdminEmail ? "هذا الحساب محمي: يمكنك تعديل الاسم فقط عند تسجيل الدخول بالحساب نفسه." : "حدّث الاسم أو البريد أو الدور أو حالة الحساب."}</p><div className="mt-4 space-y-3"><label className="block text-sm font-bold">الاسم<input className={`${fieldClass} mt-1`} value={editName} onChange={e => setEditName(e.target.value)} /></label><label className="block text-sm font-bold">البريد الإلكتروني<input type="email" dir="ltr" disabled={editing.email.trim().toLowerCase() === protectedPlatformAdminEmail} className={`${fieldClass} mt-1 disabled:cursor-not-allowed disabled:opacity-60`} value={editEmail} onChange={e => setEditEmail(e.target.value)} /></label><label className="block text-sm font-bold">الدور<select disabled={editing.email.trim().toLowerCase() === protectedPlatformAdminEmail} className={`${fieldClass} mt-1 disabled:cursor-not-allowed disabled:opacity-60`} value={editRole} onChange={e => setEditRole(e.target.value as typeof editRole)}>{roles.map(item => <option key={item} value={item}>{roleLabel[item]}</option>)}</select></label><label className="block text-sm font-bold">الحالة<select disabled={editing.email.trim().toLowerCase() === protectedPlatformAdminEmail} className={`${fieldClass} mt-1 disabled:cursor-not-allowed disabled:opacity-60`} value={editStatus} onChange={e => setEditStatus(e.target.value as typeof editStatus)}><option value="active">نشط</option><option value="disabled">معطّل</option></select></label></div><div className="mt-5 flex gap-2"><PlatformButton disabled={saving || editName.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(editEmail)} onClick={() => void saveEdit()}>{saving ? "جارٍ الحفظ…" : "حفظ التعديل"}</PlatformButton><PlatformButton variant="secondary" disabled={saving} onClick={() => setEditing(null)}>إلغاء</PlatformButton></div></div></div>}
    {formOpen && <div className="fixed inset-0 z-50 overflow-y-auto bg-slate-950/50 p-4"><div className="mx-auto mt-16 max-w-lg rounded-2xl bg-white p-5 shadow-xl dark:bg-slate-900"><h2 className="font-black">إضافة مشرف للمنصة</h2><div className="mt-4 space-y-3"><input className={fieldClass} value={name} onChange={e => setName(e.target.value)} placeholder="الاسم" /><input type="email" className={fieldClass} value={email} onChange={e => setEmail(e.target.value)} placeholder="البريد الإلكتروني" /><input type="password" className={fieldClass} value={password} onChange={e => setPassword(e.target.value)} placeholder="كلمة مرور مؤقتة — 12 حرفًا على الأقل" /><select className={fieldClass} value={role} onChange={e => setRole(e.target.value as typeof role)}>{roles.map(item => <option key={item} value={item}>{roleLabel[item]}</option>)}</select></div><div className="mt-5 flex gap-2"><PlatformButton disabled={saving || name.trim().length < 2 || !/^\S+@\S+\.\S+$/.test(email) || password.length < 12} onClick={() => void create()}>{saving ? "جارٍ الإنشاء…" : "إنشاء الحساب"}</PlatformButton><PlatformButton variant="secondary" onClick={() => setFormOpen(false)}>إلغاء</PlatformButton></div></div></div>}</section>;
}
