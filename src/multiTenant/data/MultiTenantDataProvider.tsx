...
export function MultiTenantDataProvider({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    ...
    if (allowed('company:notifications:read') && authSession?.uid) unsubs.push(listen<AppNotification>('notifications', setNotifications, role === 'company_super_admin' ? undefined : { field: 'targetUid', value: authSession.uid }));
  ...
  const updateOrder = useCallback(async (id: string, data: Partial<Order>) => {
    ...
    if (authSession?.role === 'worker') {
      const keys = Object.keys(data); const status = data.orderStatus;
      if (keys.length !== 1 || !status || !['preparing', 'in_progress', 'completed'].includes(status)) throw new Error('العامل يمكنه تحديث حالة الطلب المسند إليه فقط.');
      const result = await companyMembersService.updateWorkerOrderStatus({ orderId: id, status: status as 'preparing' | 'in_progress' | 'completed' });
      if (!result.success) throw new Error(result.message);
      setOrders(current => current.map(order => order.id === id ? { ...order, orderStatus: status, updatedAt: new Date().toISOString() } : order));
      return;
    }
  ...
  const value = useMemo<DataContextType>(() => ({ orders, customers, workers, inventory, expenses, settings, notifications, categories, activityLogs, loading, totalCapital: expenses.filter((item) => item.type === 'capital').reduce((sum, item) => sum + (item.amount || 0), 0), totalGeneralExpenses: expenses.filter((item) => item.type !== 'capital').reduce((sum, item) => sum + (item.amount || 0), 0), currentCashBalance: expenses.filter((item) => item.type === 'capital').reduce((sum, item) => sum + (item.amount || 0), 0) - expenses.filter((item) => item.type === 'capital').reduce((sum, item) => sum + (item.amount || 0), 0), addOrder, updateOrder, deleteOrder, addPaymentToOrder, addWorker: async () => { throw new Error('إنشاء العامل متاح من قسم العمال فقط.'); }, updateWorker: updateWorkerSafe, deleteWorker: deleteWorkerSafe, toggleWorkerStatus: toggleWorkerStatusSafe, addCustomer: (data) => addRecord('customers', 'cus', data), updateCustomer: (id, data) => updateRecord('customers', id, data), deleteCustomer: (id) => remove('customers', id), addInventoryItem: (data) => addRecord('inventory', 'inv', { ...data, reservedQuantity: 0, availableQuantity: data.quantity }), updateInventoryItem: (id, data) => updateRecord('inventory', id, data), deleteInventoryItem: (id) => remove('inventory', id), addExpense: (data) => { if (data.linkedOrderId && !orders.some((order) => order.id === data.linkedOrderId)) return Promise.reject(new Error('الطلب المرتبط لا يتبع الشركة الحالية.')); return addRecord('expenses', 'exp', data); }, updateExpense: (id, data) => updateRecord('expenses', id, data), deleteExpense: (id) => remove('expenses', id), addCategory: async (nameEn, nameAr) => { const category: CategoryItem = { id: newId('cat'), key: `custom_${nameEn.toLowerCase().replace(/[^a-z0-9]/g, '_')}`, nameEn: sanitizeData(nameEn), nameAr: sanitizeData(nameAr), isCustom: true }; await write('categories', category.id, category); return category; }, updateSettings, seedSampleData: async () => { throw new Error('البيانات التجريبية معطلة في وضع الشركات.'); }, exportBackupJson, restoreBackupJson, addActivityLog: addActivityLogSafe, markNotificationAsRead: async (id) => markNotificationsSafe([id]), clearAllNotifications: async () => markNotificationsSafe(notifications.filter((item) => !item.read).map((item) => item.id)), checkStockAvailability }), [activityLogs, addActivityLogSafe, addOrder, addPaymentToOrder, addRecord, categories, checkStockAvailability, customers, deleteOrder, deleteWorkerSafe, expenses, exportBackupJson, inventory, loading, markNotificationsSafe, notifications, orders, remove, restoreBackupJson, settings, toggleWorkerStatusSafe, updateOrder, updateRecord, updateSettings, updateWorkerSafe, workers, write]);
