...
import type { ActivityLogRecord, AppNotification, CategoryItem, CompanySettings, Customer, Expense, InventoryItem, Order, PaymentEntry, Supplier, Worker, WorkerMovement, WorkTask } from '../../types';
...
export function MultiTenantDataProvider({ children }: { children: React.ReactNode }) {
  ...
  const value = useMemo<DataContextType>(() => ({ orders, workTasks, customers, suppliers, workers, inventory, expenses, settings, notifications, categories, activityLogs, loading, ...financialTotals, addOrder, updateOrder, deleteOrder, addPaymentToOrder, addWorkTask, updateWorkTask, deleteWorkTask, addWorker: async () => { throw new Error('إنشاء العامل متاح من قسم العمال فقط.'); }, updateWorker: updateWorkerSafe, deleteWorker: deleteWorkerSafe, toggleWorkerStatus: toggleWorkerStatusSafe, addCustomer: (data) => addRecord('customers', 'cus', data), updateCustomer: (id, data) => updateRecord('customers', id, data), deleteCustomer: (id) => remove('customers', id), addSupplier: (data) => addRecord('suppliers', 'sup', data), updateSupplier: (id, data) => updateRecord('suppliers', id, data), deleteSupplier: (id) => remove('suppliers', id), addInventoryItem: (data) => addRecord('inventory', 'inv', { ...data, reservedQuantity: 0, availableQuantity: data.quantity }), updateInventoryItem: (id, data) => updateRecord('inventory', id, data), deleteInventoryItem: (id) => remove('inventory', id), addExpense: (data) => { if (data.linkedOrderId && !orders.some((order) => order.id === data.linkedOrderId)) return Promise.reject(new Error('الطلب المرتبط لا يتبع الشركة الحالية.')); return addRecord('expenses', 'exp', data); }, updateExpense: (id, data) => updateRecord('expenses', id, data), deleteExpense: (id) => remove('expenses', id), addCategory, updateSettings, seedSampleData: async () => { throw new Error('البيانات التجريبية معطلة في وضع الشركات.'); }, exportBackupJson, restoreBackupJson, addActivityLog: addActivityLogSafe, recordWorkerMovement: recordWorkerMovementSafe, markNotificationAsRead: async (id) => markNotificationsSafe([id]), clearAllNotifications: async () => markNotificationsSafe(notifications.filter((item) => !item.read).map((item) => item.id)), checkStockAvailability }), [activityLogs, addActivityLogSafe, addCategory, addOrder, addPaymentToOrder, addRecord, addWorkTask, categories, checkStockAvailability, customers, deleteOrder, deleteWorkTask, deleteWorkerSafe, expenses, exportBackupJson, financialTotals, inventory, loading, markNotificationsSafe, notifications, orders, recordWorkerMovementSafe, remove, restoreBackupJson, settings, suppliers, toggleWorkerStatusSafe, updateOrder, updateRecord, updateSettings, updateWorkTask, updateWorkerSafe, workTasks, workers, write]);
